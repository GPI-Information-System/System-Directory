/* ============================================================
   ...
   Features:
   - View all categories with system counts
   - Drag-and-drop reorder (all roles)
   - Inline rename (all roles)
   - Add new category (Super Admin only)
   - Delete with reassignment confirmation (Super Admin only)
   ============================================================ */

const CategoriesApp = {
    categories:       [],       // current list from server
    dragSrcIndex:     null,     // drag source index
    pendingReorder:   false,    // unsaved reorder pending
    isSuperAdmin:     false,    // set on open from PHP data attr
};
...
function openCategoriesModal() {
    ...
    // Read role from data attribute set by PHP
    ...
    if (addSection) addSection.style.display = CategoriesApp.isSuperAdmin ? 'block' : 'none';
...
function loadCategories() {
    ...
    if (!listEl) return;
    listEl.innerHTML = '<div class="cat-loading"><div class="loading-spinner" style="width:20px;height:20px;border-width:2px;"></div><span>Loading...</span></div>';

    fetch('../backend/categories/get_categories.php')
        .then(r => r.json())
        .then(data => {
            if (!data.success) {
                listEl.innerHTML = '<div class="cat-empty">Failed to load categories.</div>';
                return;
            }
        ...
        .catch(() => {
            listEl.innerHTML = '<div class="cat-empty">Network error. Please try again.</div>';
        });
}
...
function renderCategoryList() {
    ...
    listEl.innerHTML = CategoriesApp.categories.map((cat, index) => `
        <div class="cat-row"
             draggable="true"
             data-index="${index}"
             data-id="${cat.id}">
    ...
    listEl.querySelectorAll('.cat-row').forEach(row => {
        row.addEventListener('dragstart', e => onCatDragStart(e, parseInt(row.dataset.index)));
        row.addEventListener('dragover',  e => onCatDragOver(e));
        row.addEventListener('drop',      e => onCatDrop(e, parseInt(row.dataset.index)));
...
function saveReorder() {
    ...
    fetch('../backend/categories/reorder_categories.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
                CategoriesApp.pendingReorder = false;
...
function onCatRenameKey(event, catId) {
    if (event.key === 'Enter')  { event.preventDefault(); saveCatRename(catId); }
    if (event.key === 'Escape') { cancelCatRename(catId); }
}
...
function saveCatRename(catId) {
    ...
    if (!newName) { showCatToast('Category name cannot be empty', 'error'); return; }

    const cat = CategoriesApp.categories.find(c => c.id === catId);
    if (cat && cat.name === newName) { cancelCatRename(catId); return; }

    const btnSave = document.getElementById(`cat-btn-save-${catId}`);
    if (btnSave) { btnSave.disabled = true; btnSave.innerHTML = '<span style="opacity:.6">…</span>'; }
    ...
    fetch('../backend/categories/save_category.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) {
...
function addNewCategory() {
    ...
    if (!name) { showCatToast('Please enter a category name', 'error'); input.focus(); return; }
...
function onCatAddKeydown(event) {
    if (event.key === 'Enter') { event.preventDefault(); addNewCategory(); }
}
...
function confirmDeleteCategory() {
    ...
    const reassignTo = sysCount > 0 ? document.getElementById('catReassignSelect').value : '';
    ...
    fetch('../backend/categories/delete_category.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
...
// ── Refresh category dropdowns on dashboard ──────────────────
        ...
        .then(data => {
            ...
            if (addSel) {
                ...
                addSel.innerHTML = '<option value="">Select a category...</option>' +
                    cats.map(c => `<option value="${escCat(c.name)}"${c.name === current ? ' selected' : ''}>${escCat(c.name)}</option>`).join('');
            }

            const editSel = document.getElementById('editSystemCategory');
            if (editSel) {
                const current = editSel.value;
                editSel.innerHTML = '<option value="">Select a category...</option>' +
                    cats.map(c => `<option value="${escCat(c.name)}"${c.name === current ? ' selected' : ''}>${escCat(c.name)}</option>`).join('');
            }
