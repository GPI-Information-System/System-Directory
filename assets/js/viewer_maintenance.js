/* ============================================================
   ...
   Complete file: Original functions + JP translation
   ============================================================ */
...
function vmInitStickyControls() {
    ...
    const observer = new IntersectionObserver(
        ([entry]) => { sticky.classList.toggle('is-stuck', !entry.isIntersecting); },
...
function vmTickCountdown(el) {
    ...
    const label   = h > 0 ? `${endsIn} ${h}h ${m}m` : m > 0 ? `${endsIn} ${m}m ${s}s` : `${endsIn} ${s}s`;
...
function vmApplyFilters() {
    ...
    cards.forEach(card => {
        ...
        if (matchFilter && matchSearch) { card.classList.remove('vm-hidden'); visible++; }
    ...
    if (resultCount) {
        if (vmIsJapanese) {
            const statusLabelsJP = { 'In Progress': '進行中', 'Scheduled': '予定済み', 'Done': '完了' };
            let txt = `<strong>${visible}</strong>件`;
            if (vmActiveFilter !== 'all') txt += `の${statusLabelsJP[vmActiveFilter] || vmActiveFilter}`;
            txt += 'のメンテナンス記録を表示中';
            if (vmActiveSearch) txt += `（"<em>${escVm(vmActiveSearch)}</em>"に一致）`;
            if (visible < VM_TOTAL) txt += ` <span class="vm-count-total">（全${VM_TOTAL}件）</span>`;
    ...
    if (filterEmpty) {
        if (visible === 0) {
            ...
            const emptyTitles = vmIsJapanese
                ? { 'In Progress': 'アクティブなメンテナンスなし', 'Scheduled': '予定なし', 'Done': '完了記録なし', 'all': '記録が見つかりません' }
                : { 'In Progress': 'No Active Maintenance', 'Scheduled': 'Nothing Scheduled', 'Done': 'No Completed Records', 'all': 'No Records Found' };
            const emptyMsgs = vmIsJapanese
                ? { 'In Progress': '現在メンテナンス中のシステムはありません。', 'Scheduled': '現在予定されているメンテナンスはありません。', 'Done': 'まだ完了したメンテナンスはありません。', 'all': vmActiveSearch ? `"${escVm(vmActiveSearch)}"に一致する記録はありません。` : 'メンテナンス記録はありません。' }
                : { 'In Progress': 'There are no systems currently under maintenance.', 'Scheduled': 'No maintenance windows are currently scheduled.', 'Done': 'No maintenance has been completed yet.', 'all': vmActiveSearch ? `No records match "${escVm(vmActiveSearch)}".` : 'No maintenance records available.' };
            if (titleEl) titleEl.textContent = emptyTitles[vmActiveFilter] || (vmIsJapanese ? '結果が見つかりません' : 'No Results Found');
            if (msgEl)   msgEl.textContent   = emptyMsgs[vmActiveFilter]   || (vmIsJapanese ? '条件に一致するメンテナンス記録はありません。' : 'No records match your filter.');
        } else {
...
function vmApplyDonePagination() {
    ...
    if (vmDoneShowAll) {
        ...
        doneCards.forEach((c, i) => { c.classList.toggle('vm-done-hidden', i >= VM_DONE_PAGE_SIZE); });
        if (showMoreWrap) {
            ...
            if (showMoreTextEl) showMoreTextEl.textContent = vmIsJapanese
                ? `過去の記録を${hidden}件表示`
...
document.addEventListener('keydown', function (e) {
    ...
    if (toggle && document.activeElement === toggle && (e.key === 'Enter' || e.key === ' ')) {
        e.preventDefault(); vmToggleAlertDropdown();
    }
    const input = document.getElementById('vmSearch');
...
const VM_JP = {
    ...
    allClearMsg:     '現在メンテナンスの予定はありません。全システムは正常に動作しています。',
...
function vmIsJpMode() { return localStorage.getItem('gportal_jp_mode') === 'true'; }
...
function vmSetLanguage(lang) {
    ...
    // Re-apply filters so result count, empty state, show-more btn
    // all update instantly in the new language without reload
    vmApplyFilters();
}
...
function vmSet(id, text) { const el = document.getElementById(id); if (el) el.textContent = text; }
...
function vmApplyJP() {
    ...
    document.querySelectorAll('.vm-under-maint-text').forEach(el => el.textContent = VM_JP.underMaint);
    document.querySelectorAll('.vm-alert-contact').forEach(el => {
        const num = el.getAttribute('data-number') || '';
        const svg = el.querySelector('svg');
        el.innerHTML = ''; if (svg) el.appendChild(svg);
        el.appendChild(document.createTextNode(' ' + VM_JP.contact(num)));
    });
    document.querySelectorAll('.vm-pill-inprogress-text').forEach(el => el.textContent = VM_JP.pillInProgress(VM_COUNT_ACTIVE));
    document.querySelectorAll('.vm-pill-scheduled-text').forEach(el => el.textContent = VM_JP.pillScheduled(VM_COUNT_SCHEDULED));
    document.querySelectorAll('.vm-pill-done-text').forEach(el => el.textContent = VM_JP.pillCompleted(VM_COUNT_DONE));
    ...
    if (search) { search.setAttribute('data-en-ph', search.placeholder); search.placeholder = VM_JP.searchPlaceholder; }
    const barText = document.getElementById('vmAllClearBarText');
    if (barText) { barText.setAttribute('data-en', barText.textContent); barText.textContent = VM_JP.allClearBarFn(VM_COUNT_DONE); }
    const acTitle = document.getElementById('vmAllClearTitle');
    if (acTitle) { acTitle.setAttribute('data-en', acTitle.textContent); acTitle.textContent = VM_JP.allClearTitle; }
    const acMsg = document.getElementById('vmAllClearMsg');
    if (acMsg) { acMsg.setAttribute('data-en', acMsg.textContent); acMsg.textContent = VM_JP.allClearMsg; }
    vmSet('vmShowMoreText', VM_JP.showMore);
    document.querySelectorAll('.vm-status-text').forEach(el => {
        const en = el.closest('[data-status-en]')?.getAttribute('data-status-en') || el.textContent.trim();
        if (!el.getAttribute('data-en')) el.setAttribute('data-en', en);
        el.textContent = VM_JP.status[en] || en;
    });
    document.querySelectorAll('.vm-label-start').forEach(el => { el.setAttribute('data-en','Start'); el.textContent = VM_JP.labelStart; });
    document.querySelectorAll('.vm-label-end').forEach(el => { el.setAttribute('data-en','End'); el.textContent = VM_JP.labelEnd; });
    document.querySelectorAll('.vm-inprogress-text').forEach(el => { el.setAttribute('data-en', el.textContent); el.textContent = VM_JP.inProgress; });
    document.querySelectorAll('.vm-starts-text').forEach(el => { el.setAttribute('data-en', el.textContent); el.textContent = VM_JP.starts; });
    document.querySelectorAll('.vm-ontime-text').forEach(el => { el.setAttribute('data-en', el.textContent); el.textContent = VM_JP.onTime; });
    document.querySelectorAll('.vm-exceeded-text').forEach(el => {
        const en = el.getAttribute('data-en') || el.textContent;
        el.setAttribute('data-en', en); el.textContent = VM_JP.overSchedule(en);
    });
}
...
function vmRevertEN() {
    ...
    vmSet('vmPageSubtitle',     'Scheduled, ongoing, and completed maintenance windows');
    ...
    const badge = document.getElementById('vmAlertBadge');
    if (badge) badge.textContent = `${VM_COUNT_ACTIVE} system${VM_COUNT_ACTIVE !== 1 ? 's' : ''} affected`;
    const dropLabel = document.getElementById('vmDropdownLabel');
    if (dropLabel) dropLabel.textContent = `${VM_COUNT_ACTIVE} system${VM_COUNT_ACTIVE !== 1 ? 's' : ''} currently under maintenance`;
    document.querySelectorAll('.vm-under-maint-text').forEach(el => el.textContent = 'Under Maintenance');
    document.querySelectorAll('.vm-alert-contact').forEach(el => {
        const num = el.getAttribute('data-number') || '';
        const svg = el.querySelector('svg');
        el.innerHTML = ''; if (svg) el.appendChild(svg);
        el.appendChild(document.createTextNode(' Contact '));
        const strong = document.createElement('strong'); strong.textContent = num; el.appendChild(strong);
        el.appendChild(document.createTextNode(' for assistance'));
    });
    document.querySelectorAll('.vm-pill-inprogress-text').forEach(el => el.textContent = `${VM_COUNT_ACTIVE} In Progress`);
    document.querySelectorAll('.vm-pill-scheduled-text').forEach(el => el.textContent = `${VM_COUNT_SCHEDULED} Scheduled`);
    document.querySelectorAll('.vm-pill-done-text').forEach(el => el.textContent = `${VM_COUNT_DONE} Completed`);
    vmSet('vmBtnAll',           'All');
    ...
    const search = document.getElementById('vmSearch');
    if (search) { const en = search.getAttribute('data-en-ph'); if (en) search.placeholder = en; }
    const barText = document.getElementById('vmAllClearBarText');
    if (barText) { const en = barText.getAttribute('data-en'); if (en) barText.textContent = en; }
    const acTitle = document.getElementById('vmAllClearTitle');
    if (acTitle) { const en = acTitle.getAttribute('data-en'); if (en) acTitle.textContent = en; }
    const acMsg = document.getElementById('vmAllClearMsg');
    if (acMsg) { const en = acMsg.getAttribute('data-en'); if (en) acMsg.textContent = en; }
    vmSet('vmShowMoreText', 'Show older records');
    document.querySelectorAll('.vm-status-text').forEach(el => { const en = el.getAttribute('data-en'); if (en) el.textContent = en; });
    document.querySelectorAll('.vm-label-start').forEach(el => el.textContent = 'Start');
    document.querySelectorAll('.vm-label-end').forEach(el => el.textContent = 'End');
    document.querySelectorAll('.vm-inprogress-text').forEach(el => { const en = el.getAttribute('data-en'); if (en) el.textContent = en; });
    document.querySelectorAll('.vm-starts-text').forEach(el => el.textContent = 'Starts');
    document.querySelectorAll('.vm-ontime-text').forEach(el => el.textContent = 'Completed on time');
    document.querySelectorAll('.vm-exceeded-text').forEach(el => { const en = el.getAttribute('data-en'); if (en) el.textContent = en; });
}
