...
function switchUptimeView(view) {
    ...
    if (combo) combo.style.display = view === 'per-system' ? 'inline-flex' : 'none';
...
function selectUptimeSystem(id, name) {
    ...
    document.querySelectorAll('#uptimeSystemDropdown .report-system-option').forEach(opt => {
...
function clearUptimeSystemSearch() {
    ...
    document.querySelectorAll('#uptimeSystemDropdown .report-system-option').forEach(opt => opt.classList.remove('selected'));
    const dropdown = document.getElementById('uptimeSystemDropdown');
    if (dropdown) dropdown.style.display = 'none';
}
...
function loadUptimeData() {
    const systemId = currentUptimeView === 'per-system'
        ? document.getElementById('uptimeSystemSelect').value : 0;
...
// ============================================================
// FIX 1: renderUptimeChart
// Per-system: reconstructs time-based uptime from change events
// Overall: shows % of online-transitions per day as a trend
// Y-axis capped at 100%, bars color-coded green/yellow/red
// Tooltips show meaningful detail per bar
// ============================================================
function renderUptimeChart(data, systemId, responseData) {
    const ctx = document.getElementById('uptimeChart').getContext('2d');
    const dates = [], uptimePercentages = [], tooltipDetails = [];

    if (systemId > 0) {
        // Per-system: walk the event timeline to compute real seconds-online per day
        const sorted = data.slice().sort((a, b) => new Date(a.changed_at) - new Date(b.changed_at));
        const dailySeconds = {};
        const now = new Date();
        ...
        for (let i = 0; i < sorted.length; i++) {
            ...
            while (cursor < segEnd) {
                ...
                if (!dailySeconds[dayStr]) dailySeconds[dayStr] = { online: 0, total: 0 };
        ...
        Object.keys(dailySeconds).sort().forEach(date => {
        ...
        // Overall: group daily counts, show % of transitions that were online
        const dailyData = {};
        data.forEach(entry => {
    ...
    const barColors   = uptimePercentages.map(p => p >= 80 ? 'rgba(16,185,129,0.8)' : p >= 50 ? 'rgba(245,158,11,0.8)' : 'rgba(239,68,68,0.8)');
    const borderColors = uptimePercentages.map(p => p >= 80 ? 'rgba(16,185,129,1)'   : p >= 50 ? 'rgba(245,158,11,1)'   : 'rgba(239,68,68,1)');
    ...
    uptimeChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: systemId > 0 ? 'Daily Uptime % (time-based)' : 'Overall System Uptime — Last 30 Days',
...
function calculateUptimeStats(data, systemId, responseData) {
    if (systemId > 0) {
        ...
        const avgUptime = data.length > 0
            ? ((data.filter(e => e.status === 'online').length / data.length) * 100).toFixed(1)
            : 0;
        updateUptimeStats({
        ...
        const onlineEntries = data.filter(e => e.status === 'online').reduce((sum, e) => sum + parseInt(e.count), 0);
        const avgUptime     = totalEntries > 0 ? ((onlineEntries / totalEntries) * 100).toFixed(1) : 0;
        let incidents = 0;
        data.forEach(entry => {
            if (entry.status === 'down' || entry.status === 'offline') incidents += parseInt(entry.count);
        });
        updateUptimeStats({ current: currentPct + '%', average: avgUptime + '%', incidents });
    }
}
...
function loadStatusHistory(days) {
    ...
    fetch('../backend/get_analytics_data.php?action=status_history&days=' + days)
        .then(r => r.json())
        .then(data => {
            if (data.success) { allStatusLogs = data.data; applyLogSearch(); }
        })
        .catch(() => { tbody.innerHTML = '<tr><td colspan="5" class="loading-cell">Error loading data</td></tr>'; });
}

function applyLogSearch() {
    const q = logSearchQuery.toLowerCase().trim();
    filteredStatusLogs = q
        ? allStatusLogs.filter(l =>
...
function onLogSearch(input) { logSearchQuery = input.value; applyLogSearch(); }
...
function renderStatusHistory() {
    ...
    if (filteredStatusLogs.length === 0) {
        const msg = logSearchQuery ? 'No results matching "<strong>' + escapeHtml(logSearchQuery) + '</strong>"' : 'No status changes found';
    ...
    const logsToDisplay = filteredStatusLogs.slice(startIndex, startIndex + itemsPerPage);
    ...
    tbody.innerHTML = logsToDisplay.map(log => {
        ...
        return '<tr>' +
            ...
            '<td class="col-status"><div class="status-change"><span class="status-badge ' + log.old_status + '">' + capitalize(log.old_status) + '</span><span class="arrow">→</span><span class="status-badge ' + log.new_status + '">' + capitalize(log.new_status) + '</span></div></td>' +
            '<td class="col-by"><span class="truncate-cell">' + escapeHtml(log.changed_by) + '</span></td>' +
            '<td class="col-note"><div class="note-cell">' +
                '<button class="btn-view-details" onclick="openViewDetailsModal(' + log.id + ')" title="View full details"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg></button>' +
                '<span class="truncate-cell note-text" title="' + noteText + '">' + (noteText || '<em>No note</em>') + '</span>' +
                '<button class="btn-edit-note" onclick="openEditNoteModal_byId(' + log.id + ')" title="Edit note"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>Edit</button>' +
            '</div></td>' +
        '</tr>';
    }).join('');
...
function updatePaginationInfo() {
    ...
    const suffix = logSearchQuery && allStatusLogs.length !== total
        ? ' <span class="maint-filter-note">(' + allStatusLogs.length + ' total)</span>' : '';
    el.innerHTML = 'Showing ' + s + '-' + e + ' of ' + total + ' entries' + suffix;
}
...
function loadCompletedMaintenance(days) {
    ...
    fetch('../backend/get_analytics_data.php?action=completed_maintenance&days=' + days)
        .then(r => r.json())
        .then(data => {
            if (data.success) { allCompletedMaint = data.data; applyMaintSearch(); }
            else { tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">Error loading data</td></tr>'; }
        })
        .catch(() => { tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">Error loading data</td></tr>'; });
}

function applyMaintSearch() {
    const q = maintSearchQuery.toLowerCase().trim();
    filteredCompletedMaint = q
        ? allCompletedMaint.filter(m =>
...
function onMaintSearch(input) { maintSearchQuery = input.value; applyMaintSearch(); }
...
function renderCompletedMaintenance() {
    ...
    if (filteredCompletedMaint.length === 0) {
        const msg = maintSearchQuery ? 'No results matching "<strong>' + escapeHtml(maintSearchQuery) + '</strong>"' : 'No completed maintenance records found';
        tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">' + msg + '</td></tr>';
        updateMaintPaginationInfo(); return;
    }
    const totalPages = Math.ceil(filteredCompletedMaint.length / maintItemsPerPage);
    const startIndex = (maintCurrentPage - 1) * maintItemsPerPage;
    const toDisplay  = filteredCompletedMaint.slice(startIndex, startIndex + maintItemsPerPage);
    tbody.innerHTML = toDisplay.map(m => {
        const exceeded = m.exceeded_duration && parseInt(m.exceeded_duration) > 0
            ? formatExceeded(parseInt(m.exceeded_duration))
            : '<span class="no-exceeded">Within schedule</span>';
        const exceededClass = m.exceeded_duration && parseInt(m.exceeded_duration) > 0 ? 'exceeded-yes' : 'exceeded-no';
        const actualEnd = m.actual_end_datetime
            ? '<span class="actual-end-time">' + formatDateTime(m.actual_end_datetime) + '</span>'
        ...
        if (m.actual_end_datetime && m.end_datetime) {
            ...
            if (diff > 60000) {
                ...
                const rem  = mins % 60;
                earlyBadge = '<span class="early-finish-badge">' + (hrs > 0 ? hrs + 'h ' + rem + 'm' : mins + 'm') + ' early</span>';
            }
        }
        return '<tr>' +
            ...
            '<td class="col-actual-end">' + actualEnd + earlyBadge + '</td>' +
...
function updateMaintPaginationInfo() {
    ...
    const suffix = maintSearchQuery && allCompletedMaint.length !== total
        ? ' <span class="maint-filter-note">(' + allCompletedMaint.length + ' total)</span>' : '';
    el.innerHTML = 'Showing ' + s + '-' + e + ' of ' + total + ' entries' + suffix;
}
...
function renderPaginationControls(totalPages, activePage, containerId, changePageFn) {
    ...
    let html = '';
    html += '<button class="pagination-btn pagination-prev" onclick="' + changePageFn.name + '(' + (activePage - 1) + ')" ' + (activePage === 1 ? 'disabled' : '') + '><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"></polyline></svg> Previous</button>';
    ...
    if (totalPages > 1) html += '<button class="pagination-number ' + (activePage === totalPages ? 'active' : '') + '" onclick="' + changePageFn.name + '(' + totalPages + ')">' + totalPages + '</button>';
    html += '</div>';
    html += '<button class="pagination-btn pagination-next" onclick="' + changePageFn.name + '(' + (activePage + 1) + ')" ' + (activePage === totalPages ? 'disabled' : '') + '>Next <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"></polyline></svg></button>';
...
function loadMonthlyReport() {
    ...
    fetch('../backend/get_analytics_data.php?action=monthly_report&system_id=' + systemId + '&month=' + month)
        .then(r => r.json())
        .then(data => { if (data.success) { renderMonthlyReport(data.data); exportBtn.disabled = false; } else { container.innerHTML = '<div class="empty-state"><p>Error loading report</p></div>'; } })
        .catch(() => { container.innerHTML = '<div class="empty-state"><p>Error loading report</p></div>'; });
}

let reportLogData = [], reportLogPage = 1;
...
function renderReportLogTable() {
    ...
    if (!tbody) return;
    const total = reportLogData.length, totalPages = Math.ceil(total / reportLogPerPage);
    ...
    tbody.innerHTML = slice.map(change =>
        ...
        '<td><span class="status-badge ' + change.old_status + '">' + capitalize(change.old_status) + '</span><span class="arrow">→</span><span class="status-badge ' + change.new_status + '">' + capitalize(change.new_status) + '</span></td>' +
        '<td>' + escapeHtml(change.changed_by) + '</td>' +
        '<td>' + (change.change_note ? escapeHtml(change.change_note) : '<em>—</em>') + '</td>' +
        '</tr>'
    ).join('');
    if (paginInfo) { const s = total === 0 ? 0 : start + 1; const e = Math.min(start + reportLogPerPage, total); paginInfo.textContent = 'Showing ' + s + '–' + e + ' of ' + total + ' entries'; }
...
function renderMonthlyReport(reportData) {
    ...
    container.innerHTML = '<div class="report-content" id="reportContent">' +
        '<div class="report-header"><h4>' + escapeHtml(reportData.system.name) + ' — Monthly Report</h4><p class="report-period">' + monthName + '</p></div>' +
        '<div class="report-stats">' +
            '<div class="report-stat-card report-stat-card--uptime"><div class="label">Uptime Percentage</div><div class="value">' + reportData.uptime_percentage + '%</div></div>' +
            '<div class="report-stat-card report-stat-card--changes"><div class="label">Status Changes</div><div class="value">' + reportData.status_changes_count + '</div></div>' +
            '<div class="report-stat-card report-stat-card--downtime"><div class="label">Downtime Incidents</div><div class="value">' + reportData.downtime_incidents + '</div></div>' +
        '</div>' +
...
// ============================================================
// SECTION 5: STATUS TRENDS
// FIX 2: Data from status_history comes DESC — we sort dates
// ascending before building Chart.js datasets so all status
// types for the same date align to the same X-axis position.
// Zero-value statuses are hidden from legend and tooltip.
// ============================================================

function loadStatusTrends() {
    fetch('../backend/get_analytics_data.php?action=status_history&days=30')
        .then(r => r.json())
        .then(data => { if (data.success) renderStatusTrendsChart(data.data); })
...
function renderStatusTrendsChart(logs) {
    ...
    // Build daily counts — use only date portion (YYYY-MM-DD)
    const dailyCounts = {};
    logs.forEach(log => {
        const date = (log.changed_at || '').slice(0, 10);
        if (!date) return;
        if (!dailyCounts[date]) dailyCounts[date] = { online: 0, offline: 0, maintenance: 0, down: 0 };
        if (dailyCounts[date][log.new_status] !== undefined) dailyCounts[date][log.new_status]++;
    });

    // Sort dates ascending — critical so Chart.js groups bars correctly per date
    const dates = Object.keys(dailyCounts).sort();

    if (dates.length === 0) {
        if (statusTrendsChart) { statusTrendsChart.destroy(); statusTrendsChart = null; }
        return;
    }
    ...
    const datasets = statusList.map(st => ({
        ...
        // Auto-hide statuses with no data at all — keeps legend clean
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        title: ctx => 'Date: ' + ctx[0].label,
                        // Hide zero-count entries from tooltip
                        label: ctx => ctx.parsed.y === 0 ? null : ' ' + ctx.dataset.label + ': ' + ctx.parsed.y + ' change' + (ctx.parsed.y !== 1 ? 's' : '')
                    }
                }
            },
            scales: {
                ...
                y: {
                    ...
                    suggestedMax: (function() {
                        const maxVal = Math.max(...statusList.map(st => Math.max(...dates.map(d => dailyCounts[d][st] || 0))));
...
// ============================================================
// DAILY BREAKDOWN — Paginated rendering
// ============================================================

function changeDailyPage(page) {
    const total = Math.ceil(window._dailyBreakdownData.length / window._dailyPerPage);
    if (page < 1 || page > total) return;
...
function renderDailyBreakdown() {
    ...
    const statusColors = { online: '#10b981', maintenance: '#f59e0b', down: '#ef4444', offline: '#6b7280', archived: '#9ca3af' };
    tbody.innerHTML = slice.map(day => {
        const pills = statusOrder.filter(st => day.statuses[st]).map(st =>
            '<span class="db-pill db-pill--' + st + '"><span class="db-pill-dot" style="background:' + statusColors[st] + '"></span>' + capitalize(st) + ': ' + day.statuses[st].formatted + '</span>'
        ).join('');
        return '<tr><td class="db-col-date"><span class="db-date-label">' + day.label + '</span></td><td class="db-col-status"><div class="db-pills">' + pills + '</div></td></tr>';
    }).join('');
    if (infoEl) { const s = total === 0 ? 0 : start + 1; const e = Math.min(start + perPage, total); infoEl.textContent = 'Showing ' + s + '-' + e + ' of ' + total + ' days'; }
    if (ctrlEl) renderPaginationControls(totalPages, page, 'dailyPaginCtrl', changeDailyPage);
}
...
function exportReportToPDF() {
    ...
    try {
        ...
        const marginL = 20, marginR = 20, pageW = 210, pageH = 297, contentW = pageW - marginL - marginR;
        let y = 20;
        pdf.setFillColor(22, 38, 96); pdf.rect(0, 0, pageW, 36, 'F');
        pdf.setTextColor(255,255,255); pdf.setFont('helvetica','bold'); pdf.setFontSize(16); pdf.text('G-Portal', marginL, 14);
        pdf.setFontSize(11); pdf.setFont('helvetica','normal'); pdf.text('Analytics & Reports', marginL, 22);
        pdf.setFontSize(9); pdf.text('Generated: ' + new Date().toLocaleString(), marginL, 30);
        y = 46;
        ...
        titleLines.forEach(line => { pdf.text(line, marginL, y); y += 8; });
        pdf.setFont('helvetica','normal'); pdf.setFontSize(12); pdf.setTextColor(107,114,128);
        pdf.text('Monthly Report — ' + monthName, marginL, y); y += 12;
        pdf.setDrawColor(229,231,235); pdf.setLineWidth(0.5); pdf.line(marginL, y, pageW - marginR, y); y += 10;
        const uptimeVal   = document.querySelector('.report-stat-card--uptime  .value') ? document.querySelector('.report-stat-card--uptime  .value').textContent : '--';
        const changesVal  = document.querySelector('.report-stat-card--changes  .value') ? document.querySelector('.report-stat-card--changes  .value').textContent : '--';
        const downtimeVal = document.querySelector('.report-stat-card--downtime .value') ? document.querySelector('.report-stat-card--downtime .value').textContent : '--';
        ...
        cardData.forEach((card, i) => {
            ...
            pdf.setFillColor(249,250,251); pdf.setDrawColor(229,231,235); pdf.setLineWidth(0.3); pdf.roundedRect(x, y, cardW, 22, 3, 3, 'FD');
            pdf.setFillColor(...card.color); pdf.roundedRect(x, y, 3, 22, 1, 1, 'F');
            pdf.setTextColor(107,114,128); pdf.setFont('helvetica','normal'); pdf.setFontSize(8); pdf.text(card.label, x+7, y+7);
            pdf.setTextColor(15,23,42); pdf.setFont('helvetica','bold'); pdf.setFontSize(18); pdf.text(card.value, x+7, y+17);
        });
        y += 30;
        pdf.setTextColor(15,23,42); pdf.setFont('helvetica','bold'); pdf.setFontSize(12); pdf.text('Total Time in Each Status', marginL, y); y += 8;
        const statusItems  = document.querySelectorAll('.time-breakdown-item');
        const statusColors = { online:[16,185,129], offline:[107,114,128], maintenance:[245,158,11], down:[239,68,68], archived:[156,163,175] };
        statusItems.forEach(item => {
            const label = item.querySelector('.status-label') ? item.querySelector('.status-label').textContent.trim().replace(/\s+/g, ' ') : '';
            const value = item.querySelector('.time-value') ? item.querySelector('.time-value').textContent.trim() : '';
            pdf.setFillColor(249,250,251); pdf.setDrawColor(229,231,235); pdf.setLineWidth(0.3); pdf.roundedRect(marginL, y, contentW, 10, 2, 2, 'FD');
            const dotColor = statusColors[label.toLowerCase().split(' ')[0]] || [156,163,175];
            pdf.setFillColor(...dotColor); pdf.circle(marginL+7, y+5, 2, 'F');
            pdf.setTextColor(30,41,59); pdf.setFont('helvetica','normal'); pdf.setFontSize(10); pdf.text(label, marginL+13, y+6.5);
            pdf.setFont('helvetica','bold'); pdf.text(value, pageW - marginR - 2, y+6.5, { align: 'right' });
            y += 12;
        });
        y += 4;
        ...
        if (allDailyData.length > 0) {
            ...
            pdf.setDrawColor(229,231,235); pdf.line(marginL, y, pageW - marginR, y); y += 8;
            pdf.setTextColor(15,23,42); pdf.setFont('helvetica','bold'); pdf.setFontSize(12); pdf.text('Daily Status Breakdown', marginL, y); y += 8;
            const dbCols = [{ x: 0, w: 34 }, { x: 34, w: contentW - 34 }], dbHeaderH = 9;
            pdf.setFillColor(230,235,245); pdf.setDrawColor(0,0,0); pdf.setLineWidth(0.4);
            dbCols.forEach(col => pdf.rect(marginL + col.x, y, col.w, dbHeaderH, 'FD'));
            pdf.setFont('helvetica','bold'); pdf.setFontSize(8); pdf.setTextColor(30,41,59);
            pdf.text('DATE', marginL + dbCols[0].x + 2, y + 6); pdf.text('STATUS ACTIVITY', marginL + dbCols[1].x + 2, y + 6);
            y += dbHeaderH;
            const statusOrder = ['online', 'maintenance', 'down', 'offline', 'archived'];
            allDailyData.forEach((day, idx) => {
                const rowH = 9, fillColor = idx % 2 === 0 ? [255,255,255] : [248,250,252];
                pdf.setDrawColor(0,0,0); pdf.setLineWidth(0.4);
                dbCols.forEach(col => { pdf.setFillColor(...fillColor); pdf.rect(marginL + col.x, y, col.w, rowH, 'FD'); });
                ...
                statusOrder.filter(st => day.statuses[st]).forEach(st => {
                    const text = capitalize(st) + ': ' + day.statuses[st].formatted;
                    const dotColor = statusColors[st] || [156,163,175];
                    pdf.setFillColor(...dotColor); pdf.circle(pillX + 1.5, y + 4.5, 1.5, 'F');
                    pdf.setFont('helvetica','normal'); pdf.setFontSize(8.5); pdf.setTextColor(51,65,85);
                    pdf.text(text, pillX + 5, y + 6); pillX += pdf.getTextWidth(text) + 11;
                });
                y += rowH; if (y > pageH - 20) { pdf.addPage(); y = 20; }
            });
        }
        if (reportLogData.length > 0) {
            if (y > pageH - 60) { pdf.addPage(); y = 20; }
            pdf.setDrawColor(229,231,235); pdf.line(marginL, y, pageW - marginR, y); y += 8;
            pdf.setTextColor(15,23,42); pdf.setFont('helvetica','bold'); pdf.setFontSize(12); pdf.text('Status Change Log', marginL, y); y += 8;
            const cols = [{ x:0, w:46 }, { x:46, w:40 }, { x:86, w:30 }, { x:116, w: contentW - 116 }], headerH = 9;
            pdf.setFillColor(230,235,245); pdf.setDrawColor(0,0,0); pdf.setLineWidth(0.4);
            cols.forEach(col => pdf.rect(marginL + col.x, y, col.w, headerH, 'FD'));
            pdf.setFont('helvetica','bold'); pdf.setFontSize(8); pdf.setTextColor(30,41,59);
            pdf.text('DATE & TIME', marginL+cols[0].x+2, y+6); pdf.text('CHANGE', marginL+cols[1].x+2, y+6);
            pdf.text('CHANGED BY', marginL+cols[2].x+2, y+6); pdf.text('NOTE', marginL+cols[3].x+2, y+6);
            y += headerH;
            reportLogData.forEach((change, idx) => {
                ...
                cols.forEach(col => { pdf.setFillColor(...fillColor); pdf.rect(marginL + col.x, y, col.w, rowH, 'FD'); });
                pdf.setFont('helvetica','normal'); pdf.setFontSize(8.5); pdf.setTextColor(51,65,85);
                const textY = y + 6;
        ...
        for (let p = 1; p <= totalPages; p++) {
            pdf.setPage(p);
            pdf.setFillColor(249,250,251); pdf.rect(0, pageH - 12, pageW, 12, 'F');
            pdf.setDrawColor(229,231,235); pdf.line(0, pageH - 12, pageW, pageH - 12);
...
function openViewDetailsModal(id) {
    ...
    document.getElementById('vdStatusChange').innerHTML = '<span class="status-badge ' + log.old_status + '">' + capitalize(log.old_status) + '</span><span style="margin:0 6px;color:#9ca3af;font-weight:600;">→</span><span class="status-badge ' + log.new_status + '">' + capitalize(log.new_status) + '</span>';
    const noteEl = document.getElementById('vdNote');
    const note   = log.change_note || '';
    if (note.trim()) { noteEl.textContent = note; noteEl.style.whiteSpace = 'pre-wrap'; noteEl.classList.remove('vd-note-empty'); }
    else { noteEl.innerHTML = '<em style="color:#9ca3af;">No note provided</em>'; noteEl.classList.add('vd-note-empty'); }
    document.getElementById('viewDetailsModal').classList.add('show');
}

function closeViewDetailsModal() { document.getElementById('viewDetailsModal').classList.remove('show'); }
...
function saveEditedNote(event) {
    ...
    fetch('../backend/update_log_note.php', { method: 'POST', body: formData })
        .then(r => r.json())
        .then(data => {
            if (data.success) { loadStatusHistory(currentHistoryDays); closeEditNoteModal(); showSuccessMessage('Note updated successfully!'); }
            else { alert(data.message || 'Error updating note'); }
        })
...
function formatDateTime(datetime) {
    return new Date(datetime).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' });
}
...
function loadSystemsByStatus() {
    fetch('../backend/get_analytics_data.php?action=systems_by_status')
        .then(r => r.json())
        .then(data => { if (data.success && window.renderDashboardChart) window.renderDashboardChart(data.data); })
        .catch(err => console.error('Error:', err));
}

// ============================================================
// MONTHLY REPORT — Combined Search + Dropdown
// ============================================================

function showReportDropdown() {
    const dropdown = document.getElementById('reportSystemDropdown');
    if (dropdown) { dropdown.style.display = 'block'; filterReportSystems(document.getElementById('reportSystemSearch').value); }
}

function hideReportDropdown() {
    setTimeout(() => { const dropdown = document.getElementById('reportSystemDropdown'); if (dropdown) dropdown.style.display = 'none'; }, 180);
}
...
function filterReportSystems(query) {
    ...
    const q = query.toLowerCase().trim();
    let visible = 0;
    options.forEach(opt => { const matches = q === '' || opt.dataset.name.toLowerCase().includes(q); opt.style.display = matches ? 'block' : 'none'; if (matches) visible++; });
    let noResults = dropdown.querySelector('.report-no-results');
    if (visible === 0) {
        if (!noResults) { noResults = document.createElement('div'); noResults.className = 'report-no-results'; noResults.textContent = 'No systems found'; dropdown.appendChild(noResults); }
        noResults.style.display = 'block';
...
function selectReportSystem(id, name) {
    ...
    const dropdown = document.getElementById('reportSystemDropdown');
    if (dropdown) dropdown.style.display = 'none';
    document.querySelectorAll('.report-system-option').forEach(opt => { opt.classList.toggle('selected', parseInt(opt.dataset.id) === id); });
    loadMonthlyReport();
}
...
function clearReportSystemSearch() {
    ...
    if (select) select.value = '';
    if (btn)    btn.style.display = 'none';
    document.querySelectorAll('.report-system-option').forEach(opt => opt.classList.remove('selected'));
    filterReportSystems('');
    loadMonthlyReport();
}
...
document.addEventListener('click', function(e) {
    ...
    if (combo && !combo.contains(e.target)) { const dropdown = document.getElementById('reportSystemDropdown'); if (dropdown) dropdown.style.display = 'none'; }
});