...
function loadCalendarSchedules() {
  ...
  fetch(
    `../backend/maintenance/get_maintenance.php?action=calendar&month=${monthStr}`,
  )
    .then((r) => r.json())
    .then((data) => {
      if (data.success) {
...
function openMaintenanceModal(systemId, systemName) {
  ...
  if (typeof updateMarkOnlineBanner === "function")
    updateMarkOnlineBanner(systemId);
  ...
  document.getElementById("maintenanceEnd").value =
    formatDatetimeLocal(twoHoursLater);
...
function loadExistingSchedules(systemId) {
  ...
  fetch(
    `../backend/maintenance/get_maintenance.php?action=system&system_id=${systemId}`,
  )
    .then((r) => r.json())
    .then((data) => {
      if (data.success) {
        ...
        list.innerHTML =
          '<div class="schedules-empty">Could not load schedules.</div>';
      }
    })
    .catch(() => {
      list.innerHTML =
...
function renderExistingSchedules(schedules) {
  ...
  if (!schedules || schedules.length === 0) {
    list.innerHTML =
      '<div class="schedules-empty">No existing schedules for this system.</div>';
    return;
  }
...
function editExistingSchedule(scheduleId) {
  fetch(
    `../backend/maintenance/get_maintenance.php?action=single&id=${scheduleId}`,
  )
    .then((r) => r.json())
    .then((data) => {
      if (!data.success) {
      ...
      document.getElementById("maintenanceStart").value = formatDatetimeLocal(
        parseDbDatetime(s.start_datetime),
      );
      document.getElementById("maintenanceEnd").value = formatDatetimeLocal(
        parseDbDatetime(s.end_datetime),
      );
...
function saveMaintenanceSchedule(event) {
  ...
  if (action === "create") {
    const existingItems = document.querySelectorAll(
      "#existingSchedulesList .existing-schedule-item",
    );
    for (const item of existingItems) {
      ...
      if (badge) {
        ...
        if (badgeText === "Scheduled" || badgeText === "In Progress") {
          showToast(
            "This system already has an active maintenance schedule. Please complete or delete it before creating a new one.",
            "error",
          );
          return;
        }
  ...
  saveBtn.innerHTML =
    '<span class="loading-spinner" style="width:16px;height:16px;border-width:2px;display:inline-block;"></span> Saving...';
  ...
  fetch(`../backend/maintenance/get_maintenance.php?action=day&date=${dateStr}`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        body.innerHTML =
          '<div class="side-panel-empty"><p>Could not load schedules.</p></div>';
      }
    })
    .catch(() => {
      body.innerHTML =
        '<div class="side-panel-empty"><p>Network error.</p></div>';
    });
}
    ...
    .map((s) => {
      ...
      const isExceeded = isFrozen
        ...
        : statusClean === "In Progress" &&
          new Date() > parseDbDatetime(s.end_datetime);
      const exceededText = isExceeded
        ? isFrozen
          ? formatStoredDuration(s.exceeded_duration)
          : ""
        : "";
      return `
...
document.addEventListener("click", function (e) {
  ...
  if (cardBtn && !e.target.closest("button")) {
    const schedule = JSON.parse(
      cardBtn.dataset.schedule.replace(/&#39;/g, "'"),
    );
    fetch(
      `../backend/maintenance/get_maintenance.php?action=single&id=${schedule.id}`,
    )
      .then((r) => r.json())
      .then((data) => {
  ...
  if (editPanelBtn) {
    editFromSidePanel(
      editPanelBtn.dataset.id,
      editPanelBtn.dataset.systemId,
      editPanelBtn.dataset.systemName,
    );
    return;
  }
});
...
function startExceededCountdowns() {
  ...
  function getLiveEls() {
    return Array.from(document.querySelectorAll(".spc-countdown")).filter(
      (el) => el.dataset.frozen === "false",
    );
  }
...
window.onclick = function (event) {
  ...
  const deleteMaintenanceModal = document.getElementById(
...
function updateBulkBar() {
  ...
  document.getElementById("bulkSelectedPlural").textContent =
    count === 1 ? "" : "s";

  const btn = document.getElementById("bulkSelectAllBtn");
  const label = document.getElementById("bulkSelectAllLabel");
  if (btn && label) {
    const totalSelectable = document.querySelectorAll(
      ".system-card:not(.bulk-excluded)",
    ).length;
  ...
  document.getElementById("bulkProceedCount").textContent =
    nonMaintenanceCount > 0 ? nonMaintenanceCount : count;
  ...
  if (allMaintenance) {
    ...
    if (markOnlineBtn) {
      markOnlineBtn.style.display = "flex";
      markOnlineBtn.disabled = false;
    }
...
function clearBulkSelection() {
  ...
  document
    .querySelectorAll(".system-card.bulk-selected")
...
async function openBulkModal() {
  ...
  function updateSystemsDisplay() {
    document.getElementById("bulkModalCount").textContent =
      BulkSchedule.modalSnapshot.ids.length;
    const chipContainer = document.getElementById("bulkModalSystemsList");
    chipContainer.innerHTML = BulkSchedule.modalSnapshot.ids
      .map((id) => {
        const name =
          BulkSchedule.modalSnapshot.names[String(id)] || `System #${id}`;
  ...
  document
    .getElementById("bulkMaintenanceForm")
    .classList.remove("bulk-form-blocked");
  const now = new Date();
  const twoHoursLater = new Date(now.getTime() + 2 * 60 * 60 * 1000);
  ...
  for (const systemId of snapshot.ids) {
    ...
    try {
      const response = await fetch(
        "../backend/maintenance/save_maintenance.php",
      ...
      if (
        ...
        snapshot.conflictTitles[systemId] = match
          ? match[1]
          : "existing schedule";
      }
    } catch (e) {
...
function showBulkConflictBanner() {
  ...
  const conflictTags = Array.from(snapshot.conflictIds)
    .map((id) => {
      const name = snapshot.names[String(id)] || `System #${id}`;
      const sched = snapshot.conflictTitles[id]
        ? ` · "${escapeHtml(snapshot.conflictTitles[id])}"`
        : "";
  ...
  if (allConflict) {
    titleEl.textContent =
      "Cannot schedule — all selected systems have active maintenance";
...
async function saveBulkMaintenance(event) {
  ...
  saveBtn.disabled = true;
  saveBtn.innerHTML =
    '<span class="loading-spinner" style="width:16px;height:16px;border-width:2px;display:inline-block;margin-right:8px;vertical-align:middle;"></span> Saving...';
  const results = [];
  for (const systemId of snapshot.ids) {
    const systemName =
      snapshot.names[String(systemId)] || `System #${systemId}`;
    const formData = new FormData();
    formData.append("action", "create");
    formData.append("system_id", systemId);
    ...
    try {
      ...
      results.push({
        ...
        message:
          data.message ||
          (data.success ? "Scheduled successfully" : "Error occurred"),
      });
    } catch (err) {
      results.push({
        systemId,
  ...
  const skippedCount = results.filter(
    (r) => !r.success && r.message.toLowerCase().includes("active"),
  ).length;
  ...
  if (successCount > 0) {
    ...
    if (successCount === results.length) {
      ...
      if (skippedCount > 0 && errorCount === 0) {
        showToast(
          `${successCount} saved, ${skippedCount} skipped (already have active schedules).`,
          "success",
        );
      } else {
        showToast(
          `${successCount} saved, ${results.length - successCount} skipped/failed.`,
          "success",
        );
      }
      setTimeout(() => {
...
function showBulkResultsInModal(results) {
  ...
  list.innerHTML = results
    .map((r) => {
      const isSkipped =
        !r.success && r.message.toLowerCase().includes("active");
...
function toggleSelectAll() {
  ...
  const allSelected =
    BulkSchedule.selectedIds.size >= totalSelectable && totalSelectable > 0;
  if (allSelected) {
    clearBulkSelection();
  } else {
...
async function fetchEmailSuggestions(q) {
  try {
    const res = await fetch(
      `../backend/maintenance/email_recipients_api.php?action=suggest&q=${encodeURIComponent(q)}`,
    );
...
function renderSuggestions(
  ...
  box.innerHTML = suggestions
    .map((s, i) => {
      ...
      const name = s.name
        ? `<div class="email-suggestion-name">${escapeHtml(s.name)}</div>`
        : "";
      const count =
        s.use_count > 0
          ? `<span class="email-suggestion-count">Used ${s.use_count}×</span>`
          : "";
...
function selectEmailSuggestion(event, email, wrapperId, tagsId, suggestionsId) {
  ...
  const input = document.getElementById(
    wrapperId === "emailTagWrapper"
      ? "emailRecipientInput"
      : "bulkEmailRecipientInput",
  );
...
function onEmailKeydown(event, input, wrapperId, tagsId, suggestionsId) {
  ...
  if (event.key === "Enter" || event.key === ",") {
    event.preventDefault();
    const active = box
      ? box.querySelector(".email-suggestion-item.active")
      : null;
    if (active) {
...
async function _bulkMarkAllOnline(snapshot) {
  ...
  for (const systemId of snapshot.ids) {
    ...
    try {
      ...
      const activeSchedule = (data.data || []).find(
      ...
      if (!activeSchedule) {
        ...
        const name = card
          ...
          : systemName;
        const domain = card
          ? card.querySelector(".card-domain")?.textContent.trim()
          : "";
        ...
        const r2 = await fetch("../backend/edit_system.php", {
          method: "POST",
          body: fd,
        });
      ...
      const r3 = await fetch("../backend/maintenance/save_maintenance.php", {
        method: "POST",
        body: fd,
      });
  ...
  results.forEach((r) => {
    if (r.success) {
      successCount++;
      const card = document.querySelector(
        `.system-card[data-system-id="${r.systemId}"]`,
      );
      if (card) {
  ...
  if (successCount === results.length) {
    showToast(
      `${successCount} system${successCount > 1 ? "s" : ""} marked as Online!`,
      "success",
    );
  } else {
    const failed = results.length - successCount;
    showToast(
      `${successCount} marked Online, ${failed} failed.`,
...
async function openBulkMarkOnlineModal() {
  ...
  for (const systemId of maintenanceIds) {
    ...
    const systemName =
      BulkSchedule.selectedNames[String(systemId)] || `System #${systemId}`;
    try {
      const res = await fetch(
        `../backend/maintenance/get_maintenance.php?action=system&system_id=${systemId}`,
      );
  ...
  listEl.innerHTML = systemSchedules
    .map(
      (item) => `
...
async function confirmBulkMarkOnline() {
  ...
  const checkedIds = Array.from(
    document.querySelectorAll(".bmo-system-check:checked"),
  ...
  for (const systemId of checkedIds) {
    ...
    try {
      if (item.schedule) {
        ...
        const r = await fetch("../backend/maintenance/save_maintenance.php", {
          method: "POST",
          body: fd,
        });
        ...
        const r = await fetch("../backend/edit_system.php", {
          method: "POST",
          body: fd,
        });
        const res = await r.json();
