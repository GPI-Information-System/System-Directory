...
function openMaintenanceModal(systemId, systemName) {
    ...
    // ── FIX Issue 1: Show/hide "Mark as Online" banner based on live system status ──
    if (typeof updateMarkOnlineBanner === 'function') updateMarkOnlineBanner(systemId);
...
function closeMaintenanceModal() {
    ...
    MaintenanceApp.editingScheduleId = null;
    _setStartDateLock(false); // Always reset lock on close
        ...
        <div class="existing-schedule-item">
            ...
            <div style="display:flex;align-items:center;gap:6px;">
                ...
                <div class="existing-schedule-actions">
                    <button class="btn-sched-edit" data-action="edit-schedule" data-id="${s.id}" title="Edit">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                        </svg>
                    </button>
                    <button class="btn-sched-delete" data-action="delete-schedule" data-id="${s.id}" data-title="${escapeHtml(s.title)}" title="Delete">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
function editExistingSchedule(scheduleId) {
    fetch(`../backend/maintenance/get_maintenance.php?action=single&id=${scheduleId}`)
        .then(r => r.json())
        .then(data => {
            if (!data.success) { showToast('Could not load schedule.', 'error'); return; }
            const s = data.data;
            ...
            // Lock start date if maintenance is already In Progress
            _setStartDateLock(s.status === 'In Progress');
        })
        .catch(() => showToast('Error loading schedule.', 'error'));
}

// Lock/unlock the Start Date & Time field
function _setStartDateLock(locked) {
    const startInput = document.getElementById('maintenanceStart');
    const startGroup = startInput ? startInput.closest('.form-group') : null;
    if (!startInput) return;

    if (locked) {
        // Use readonly instead of disabled so the value still submits with the form
        startInput.readOnly = true;
        startInput.style.cursor    = 'not-allowed';
        startInput.style.opacity   = '0.75';
        startInput.style.background = 'var(--gray-100)';
        startInput.style.color      = 'var(--gray-400)';

        // Also add a hidden backup field in case readonly isn't enough
        if (startGroup && !startGroup.querySelector('#maintenanceStartHidden')) {
            const hidden = document.createElement('input');
            hidden.type  = 'hidden';
            hidden.name  = 'start_datetime';
            hidden.id    = 'maintenanceStartHidden';
            hidden.value = startInput.value;
            startGroup.appendChild(hidden);
        }

        if (startGroup && !startGroup.querySelector('.start-lock-note')) {
            const note = document.createElement('div');
            note.className = 'start-lock-note';
            note.innerHTML = `
                ...
                Start time is locked — maintenance is already in progress
            `;
            startGroup.appendChild(note);
        }
    } else {
        startInput.readOnly        = false;
        startInput.style.cursor    = '';
        startInput.style.opacity   = '';
        startInput.style.background = '';
        startInput.style.color      = '';

        if (startGroup) {
            const note   = startGroup.querySelector('.start-lock-note');
            const hidden = startGroup.querySelector('#maintenanceStartHidden');
            if (note)   note.remove();
...
function saveMaintenanceSchedule(event) {
    ...
    if (end <= start) { showToast('End date/time must be after start date/time.', 'error'); return; }
    if (action === 'create') {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            loadCalendarSchedules();
            if (data.system_switched) { setTimeout(() => location.reload(), 800); }
        } else {
...
function confirmDeleteMaintenance() {
    ...
    fetch('../backend/maintenance/delete_maintenance.php', { method: 'POST', body: formData })
    .then(r => r.json())
    .then(data => {
        ...
        if (data.success) {
            ...
            loadCalendarSchedules();
            if (MaintenanceApp.sidePanelDate) { loadSidePanelSchedules(MaintenanceApp.sidePanelDate); }
            if (MaintenanceApp.currentSystemId) { loadExistingSchedules(MaintenanceApp.currentSystemId); }
        } else {
            showToast(data.message || 'Error deleting schedule.', 'error');
        }
    })
    .catch(() => { closeDeleteMaintenanceModal(); showToast('Network error.', 'error'); });
}
...
function loadSidePanelSchedules(dateStr) {
    ...
    body.innerHTML = `<div class="side-panel-loading"><div class="loading-spinner"></div><span>Loading schedules...</span></div>`;
    fetch(`../backend/maintenance/get_maintenance.php?action=day&date=${dateStr}`)
        .then(r => r.json())
        .then(data => {
            if (data.success) { renderSidePanelSchedules(data.data); }
            else { body.innerHTML = '<div class="side-panel-empty"><p>Could not load schedules.</p></div>'; }
        })
        .catch(() => { body.innerHTML = '<div class="side-panel-empty"><p>Network error.</p></div>'; });
}

function renderSidePanelSchedules(schedules) {
    const body = document.getElementById('sidePanelBody');
    if (!schedules || schedules.length === 0) {
        body.innerHTML = `<div class="side-panel-empty"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="4" width="18" height="18" rx="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg><p>No maintenance scheduled for this date.</p></div>`;
        return;
    }
    body.innerHTML = schedules.map(s => {
        ...
        const exceededText = isExceeded ? (isFrozen ? formatStoredDuration(s.exceeded_duration) : '') : '';
        return `
        <div class="side-panel-schedule-card ${isExceeded ? 'spc-exceeded' : ''}"
            ...
            <div class="spc-time">
                ...
                <span>${formatDateDisplay(s.start_datetime)}</span><span>→</span><span>${formatDateDisplay(s.end_datetime)}</span>
            </div>
            ${isExceeded ? `<div class="spc-exceeded-banner"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg><span>Exceeded by <span class="spc-countdown" data-end="${s.end_datetime}" data-frozen="${isFrozen ? 'true' : 'false'}">${exceededText}</span></span></div>` : ''}
            ${s.description ? `<div class="spc-description">${escapeHtml(s.description)}</div>` : ''}
            <div class="spc-footer">
                <div class="spc-created-by"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>${escapeHtml(s.created_by_name)}</div>
                <div class="spc-actions">
                    <button class="spc-btn-edit" data-action="edit-from-panel" data-id="${s.id}" data-system-id="${s.system_id}" data-system-name="${escapeHtml(s.system_name)}">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        Edit
                    </button>
                    <button class="spc-btn-delete" data-action="delete-schedule" data-id="${s.id}" data-title="${escapeHtml(s.title)}">
...
function editFromSidePanel(scheduleId, systemId, systemName) {
    ...
    // Show "Mark as Online" banner if system is currently in maintenance
    if (typeof updateMarkOnlineBanner === 'function') updateMarkOnlineBanner(systemId);
...
document.addEventListener('click', function (e) {
    ...
    if (cardBtn && !e.target.closest('button')) {
        ...
        fetch(`../backend/maintenance/get_maintenance.php?action=single&id=${schedule.id}`)
            .then(r => r.json())
            .then(data => {
                if (data.success && data.data) { openScheduleDetailModal(data.data); }
                else { openScheduleDetailModal(schedule); }
            })
            .catch(() => openScheduleDetailModal(schedule));
        return;
    }
    const editBtn = e.target.closest('[data-action="edit-schedule"]');
    if (editBtn) { editExistingSchedule(editBtn.dataset.id); return; }
    const deleteBtn = e.target.closest('[data-action="delete-schedule"]');
    if (deleteBtn) { promptDeleteMaintenance(deleteBtn.dataset.id, deleteBtn.dataset.title); return; }
    const editPanelBtn = e.target.closest('[data-action="edit-from-panel"]');
    if (editPanelBtn) { editFromSidePanel(editPanelBtn.dataset.id, editPanelBtn.dataset.systemId, editPanelBtn.dataset.systemName); return; }
});
...
function openScheduleDetailModal(s) {
    ...
    if (isExceeded) {
        if (isFrozen) {
            exceededHtml = `<div class="sdm-exceeded-block"><div class="sdm-exceeded-label"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>Exceeded Downtime</div><div class="sdm-countdown sdm-countdown-frozen">${formatStoredDuration(s.exceeded_duration)}</div></div>`;
        } else {
            exceededHtml = `<div class="sdm-exceeded-block"><div class="sdm-exceeded-label"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>Exceeded Downtime</div><div class="sdm-countdown" id="sdmCountdown">${formatExceededDuration(s.end_datetime)}</div></div>`;
        }
    }
    const html = `
        <div class="sdm-row"><div class="sdm-label">System</div><div class="sdm-value sdm-system-name"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg><span>${escapeHtml(s.system_name)}</span></div></div>
        <div class="sdm-row"><div class="sdm-label">Status</div><div class="sdm-value"><span class="maint-status-badge ${getStatusClass(s.status)}">${escapeHtml(s.status)}</span></div></div>
        <div class="sdm-row"><div class="sdm-label">Start Time</div><div class="sdm-value">${formatDateDisplay(s.start_datetime)}</div></div>
        <div class="sdm-row"><div class="sdm-label">End Time</div><div class="sdm-value ${isExceeded ? 'sdm-value-danger' : ''}">${formatDateDisplay(s.end_datetime)}</div></div>
        ${s.description ? `<div class="sdm-row"><div class="sdm-label">Description</div><div class="sdm-value">${escapeHtml(s.description)}</div></div>` : ''}
        <div class="sdm-row"><div class="sdm-label">Scheduled By</div><div class="sdm-value">${escapeHtml(s.created_by_name)}</div></div>
        ${exceededHtml}
    `;
    ...
    if (isExceeded && !isFrozen) {
        MaintenanceApp._detailCountdownInterval = setInterval(() => {
            ...
            if (!el) { clearInterval(MaintenanceApp._detailCountdownInterval); MaintenanceApp._detailCountdownInterval = null; return; }
...
function startExceededCountdowns() {
    ...
    function getLiveEls() {
        return Array.from(document.querySelectorAll('.spc-countdown')).filter(el => el.dataset.frozen === 'false');
    }
    if (getLiveEls().length === 0) return;
    function tick() {
        const els = getLiveEls();
        if (els.length === 0) { clearInterval(MaintenanceApp._panelCountdownInterval); MaintenanceApp._panelCountdownInterval = null; return; }
        els.forEach(el => { el.textContent = formatExceededDuration(el.dataset.end); });
    }
    tick();
...
function setSaveButtonMode(mode) {
    ...
    if (mode === 'update') {
        btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Update Schedule`;
    } else {
        btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg> Save Schedule`;
    }
}
...
function getStatusClass(status) {
    const map = { 'Scheduled': 'maint-status-scheduled', 'In Progress': 'maint-status-in-progress', 'Done': 'maint-status-done' };
...
window.onclick = function (event) {
    ...
    if (maintenanceModal && event.target === maintenanceModal) { closeMaintenanceModal(); }
    const deleteMaintenanceModal = document.getElementById('deleteMaintenanceModal');
    if (deleteMaintenanceModal && event.target === deleteMaintenanceModal) { closeDeleteMaintenanceModal(); }
    const bulkModal = document.getElementById('bulkMaintenanceModal');
    if (bulkModal && event.target === bulkModal) { requestCloseBulkModal(); }
};
...
function updateBulkBar() {
    ...
    // Count how many selected systems are in maintenance
    let maintenanceCount = 0;
    let nonMaintenanceCount = 0;
    BulkSchedule.selectedIds.forEach(id => {
        const card = document.querySelector(`.system-card[data-system-id="${id}"]`);
        const status = card ? card.dataset.status : 'online';
        if (status === 'maintenance') maintenanceCount++;
        else nonMaintenanceCount++;
    });

    const proceedBtn    = document.getElementById('bulkProceedBtn');
    const markOnlineBtn = document.getElementById('bulkBarMarkOnlineBtn');

    // Update Schedule button count
    document.getElementById('bulkProceedCount').textContent = nonMaintenanceCount > 0 ? nonMaintenanceCount : count;

    if (count === 0) {
        if (proceedBtn)    proceedBtn.disabled = true;
        if (markOnlineBtn) markOnlineBtn.style.display = 'none';
        return;
    }

    const allMaintenance  = maintenanceCount === count;
    const someMaintenance = maintenanceCount > 0 && nonMaintenanceCount > 0;

    if (allMaintenance) {
        // All in maintenance — show only Mark as Online, hide Schedule
        if (proceedBtn)    { proceedBtn.style.display = 'none'; proceedBtn.disabled = true; }
        if (markOnlineBtn) { markOnlineBtn.style.display = 'flex'; markOnlineBtn.disabled = false; }
        _updateBarMarkOnlineCount(maintenanceCount);
    } else if (someMaintenance) {
        // Mixed — show both buttons
        if (proceedBtn)    { proceedBtn.style.display = 'flex'; proceedBtn.disabled = false; }
        if (markOnlineBtn) { markOnlineBtn.style.display = 'flex'; markOnlineBtn.disabled = false; }
        _updateBarMarkOnlineCount(maintenanceCount);
        document.getElementById('bulkProceedCount').textContent = nonMaintenanceCount;
    } else {
        // No maintenance — show only Schedule button
        if (proceedBtn)    { proceedBtn.style.display = 'flex'; proceedBtn.disabled = false; }
        if (markOnlineBtn) markOnlineBtn.style.display = 'none';
...
function _updateBarMarkOnlineCount(count) {
    const countEl = document.getElementById('bulkBarMarkOnlineCount');
...
async function openBulkModal() {
    if (BulkSchedule.selectedIds.size === 0) { showToast('Please select at least one system.', 'error'); return; }
    ...
    function updateSystemsDisplay() {
        ...
        chipContainer.innerHTML = BulkSchedule.modalSnapshot.ids.map(id => {
            ...
            const conflictTitle = hasConflict ? `title="⚠ Already has active schedule: ${escapeHtml(BulkSchedule.modalSnapshot.conflictTitles[id] || '')}"` : `title="${escapeHtml(name)}"`;
            const warnIcon = hasConflict ? `<span class="chip-warn-icon"><svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L1 21h22L12 2zm0 3.5L20.5 19h-17L12 5.5zM11 10v4h2v-4h-2zm0 6v2h2v-2h-2z"/></svg></span>` : '';
            return `<span class="bulk-system-chip${conflictClass}" ${conflictTitle}><span class="bulk-system-chip-dot"></span>${escapeHtml(name)}${warnIcon}</span>`;
        }).join('');
    }
    if (modal.classList.contains('show')) { updateSystemsDisplay(); return; }
...
async function checkBulkConflicts(onComplete) {
    ...
    for (const systemId of snapshot.ids) {
        ...
        try {
            const response = await fetch('../backend/maintenance/save_maintenance.php', { method: 'POST', body: formData });
...
function showBulkConflictBanner() {
    ...
    if (!snapshot || snapshot.conflictIds.size === 0) { if (banner) banner.style.display = 'none'; return; }
    ...
    if (allConflict) {
        ...
        // Show "Mark All as Online" button — replaces Save when all blocked
        _showBulkMarkOnlineBtn(snapshot);
    } else {
        ...
        // Hide mark-all-online btn if not all conflict
        const existingMarkOnlineBtn = document.getElementById('bulkMarkOnlineBtn');
        if (existingMarkOnlineBtn) existingMarkOnlineBtn.remove();
    }
...
function closeBulkModal() {
    ...
    // Clean up mark-all-online button
    const markOnlineBtn = document.getElementById('bulkMarkOnlineBtn');
    if (markOnlineBtn) markOnlineBtn.remove();
...
async function saveBulkMaintenance(event) {
    ...
    if (!title) { showToast('Please enter a maintenance title.', 'error'); return; }
    if (new Date(end) <= new Date(start)) { showToast('End date/time must be after start date/time.', 'error'); return; }
    const snapshot = BulkSchedule.modalSnapshot || { ids: Array.from(BulkSchedule.selectedIds), names: Object.assign({}, BulkSchedule.selectedNames) };
    if (snapshot.ids.length === 0) { showToast('No systems selected.', 'error'); return; }
    const saveBtn  = document.getElementById('bulkSaveBtn');
    const origHTML = saveBtn.innerHTML;
    saveBtn.disabled  = true;
    ...
    for (const systemId of snapshot.ids) {
        ...
        if (bulkEmailToggle && bulkEmailToggle.checked) {
            ...
            if (emailList.length > 0) { formData.append('send_email', 'yes'); formData.append('email_recipients', JSON.stringify(emailList)); }
        }
        try {
            const response = await fetch('../backend/maintenance/save_maintenance.php', { method: 'POST', body: formData });
            const data = await response.json();
            results.push({ systemId, systemName, success: data.success, message: data.message || (data.success ? 'Scheduled successfully' : 'Error occurred') });
        } catch (err) {
    ...
    if (successCount > 0) {
        ...
        if (successCount === results.length) {
            ...
            if (skippedCount > 0 && errorCount === 0) { showToast(`${successCount} saved, ${skippedCount} skipped (already have active schedules).`, 'success'); }
            else { showToast(`${successCount} saved, ${results.length - successCount} skipped/failed.`, 'success'); }
            setTimeout(() => { closeBulkModal(); if (BulkSchedule.active) toggleBulkMode(); location.reload(); }, 3000);
        }
    } else {
        showBulkResultsInModal(results);
...
function showBulkResultsInModal(results) {
    ...
    list.innerHTML = results.map(r => {
        ...
        if (r.success) { iconClass = 'success'; msgClass = 'success'; statusText = 'Saved'; }
        else if (isSkipped) { iconClass = 'skipped'; msgClass = 'skipped'; statusText = 'Skipped — has active schedule'; }
        else { iconClass = 'error'; msgClass = 'error'; statusText = 'Error'; }
        ...
        return `<div class="bulk-result-item"><div class="bulk-result-icon ${iconClass}">${icon}</div><span class="bulk-result-name" title="${escapeHtml(r.systemName)}">${escapeHtml(r.systemName)}</span><span class="bulk-result-msg ${msgClass}">${statusText}</span></div>`;
    }).join('');
    container.style.display = 'block';
    const modalBody = container.closest('.modal-body');
    if (modalBody) { setTimeout(() => { container.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 100); }
}

function toggleSelectAll() {
    const allCards = Array.from(document.querySelectorAll('.system-card:not(.bulk-excluded)')).filter(card => card.offsetParent !== null);
...
function requestCloseBulkModal() {
    if (bulkModalHasData()) {
        document.getElementById('bulkCancelConfirm').classList.add('visible');
    } else {
        closeBulkModal();
    }
}
...
function dismissBulkConfirm() {
    document.getElementById('bulkCancelConfirm').classList.remove('visible');
}
...
document.addEventListener('click', function(e) {
    ...
    if (overlay && e.target === overlay && overlay.classList.contains('visible')) {
        dismissBulkConfirm();
    }
});
...
function _getTagState(wrapperId) {
    if (!_emailTagState[wrapperId]) { _emailTagState[wrapperId] = { tags: [], suggestionIndex: -1 }; }
...
function toggleEmailNotify(areaId) {
    ...
    if (!isVisible) { const input = area.querySelector('.email-tag-text-input'); if (input) setTimeout(() => input.focus(), 50); }
}
...
function addEmailTag(email, wrapperId, tagsId) {
    ...
    tag.innerHTML = `${escapeHtml(email)}<button type="button" class="email-tag-remove" onclick="removeEmailTag('${escapeHtml(email)}', '${wrapperId}', '${tagsId}')" title="Remove">×</button>`;
...
function removeEmailTag(email, wrapperId, tagsId) {
    ...
    Array.from(tagsEl.querySelectorAll('.email-tag')).forEach(el => {
        if (el.title === email || el.textContent.trim().startsWith(email)) { el.remove(); }
    });
}

function getEmailTags(wrapperId) { return [..._getTagState(wrapperId).tags]; }
...
function renderSuggestions(suggestions, input, wrapperId, tagsId, suggestionsId) {
    ...
    box.innerHTML = suggestions.map((s, i) => {
        ...
        return `<div class="email-suggestion-item" data-email="${escapeHtml(s.email)}" data-index="${i}" onmousedown="selectEmailSuggestion(event, '${escapeHtml(s.email)}', '${wrapperId}', '${tagsId}', '${suggestionsId}')"><div class="email-suggestion-avatar">${initial}</div><div class="email-suggestion-info"><div class="email-suggestion-email">${escapeHtml(s.email)}</div>${name}</div>${count}</div>`;
    }).join('');
...
function selectEmailSuggestion(event, email, wrapperId, tagsId, suggestionsId) {
    event.preventDefault();
    const input = document.getElementById(wrapperId === 'emailTagWrapper' ? 'emailRecipientInput' : 'bulkEmailRecipientInput');
...
function onEmailInput(input, wrapperId, tagsId, suggestionsId) {
    ...
    if (q.length < 1) { document.getElementById(suggestionsId).style.display = 'none'; return; }
...
function onEmailKeydown(event, input, wrapperId, tagsId, suggestionsId) {
    ...
    if (event.key === 'Enter' || event.key === ',') {
        ...
        if (active) { addEmailTag(active.dataset.email, wrapperId, tagsId); input.value = ''; box.style.display = 'none'; }
        else if (input.value.trim()) { addEmailTag(input.value.trim(), wrapperId, tagsId); input.value = ''; if (box) box.style.display = 'none'; }
        return;
    }
    if (event.key === 'Backspace' && !input.value) {
        const tagState = _getTagState(wrapperId);
        if (tagState.tags.length > 0) { const last = tagState.tags[tagState.tags.length - 1]; removeEmailTag(last, wrapperId, tagsId); }
        return;
    }
    if (event.key === 'ArrowDown') { event.preventDefault(); state.suggestionIndex = Math.min(state.suggestionIndex + 1, items.length - 1); items.forEach((el, i) => el.classList.toggle('active', i === state.suggestionIndex)); return; }
    if (event.key === 'ArrowUp') { event.preventDefault(); state.suggestionIndex = Math.max(state.suggestionIndex - 1, -1); items.forEach((el, i) => el.classList.toggle('active', i === state.suggestionIndex)); return; }
    if (event.key === 'Escape') { if (box) box.style.display = 'none'; }
}

function onEmailBlur(input, wrapperId, tagsId, suggestionsId) {
    setTimeout(() => {
        const v = input.value.trim();
        if (v) { addEmailTag(v, wrapperId, tagsId); input.value = ''; }
        const box = document.getElementById(suggestionsId);
...
/* ============================================================
   BULK MARK ALL AS ONLINE
   Shown in bulk modal footer when ALL selected systems
   have active maintenance schedules (all-conflict state).
   ============================================================ */

function _showBulkMarkOnlineBtn(snapshot) {
    // Remove any existing button first
    const existing = document.getElementById('bulkMarkOnlineBtn');
    if (existing) existing.remove();

    const footer = document.querySelector('#bulkMaintenanceModal .modal-footer');
    if (!footer) return;

    const btn = document.createElement('button');
    btn.type      = 'button';
    btn.id        = 'bulkMarkOnlineBtn';
    btn.className = 'btn btn-bulk-mark-online';
    btn.innerHTML = `
        ...
        Mark All as Online
    `;
    btn.onclick = () => _bulkMarkAllOnline(snapshot);
...
async function _bulkMarkAllOnline(snapshot) {
    if (!snapshot || snapshot.ids.length === 0) return;

    const btn = document.getElementById('bulkMarkOnlineBtn');
    if (btn) {
        btn.disabled  = true;
        btn.innerHTML = `
            <span class="loading-spinner"
                  style="width:14px;height:14px;border-width:2px;display:inline-block;
    ...
    for (const systemId of snapshot.ids) {
        ...
        try {
            // Step 1: Get active schedule for this system
            const res  = await fetch(`../backend/maintenance/get_maintenance.php?action=system&system_id=${systemId}`);
            const data = await res.json();
            ...
            if (!activeSchedule) {
                // No active schedule — just update system status directly
                const card   = document.querySelector(`.system-card[data-system-id="${systemId}"]`);
                const name   = card ? card.querySelector('.card-title')?.textContent.trim()  : systemName;
                const domain = card ? card.querySelector('.card-domain')?.textContent.trim() : '';

                const fd = new FormData();
                fd.append('id',          systemId);
                fd.append('name',        name);
                fd.append('domain',      domain);
                fd.append('status',      'online');
                fd.append('change_note', 'Bulk marked as Online after maintenance.');

                const r2   = await fetch('../backend/edit_system.php', { method: 'POST', body: fd });
                const res2 = await r2.json();
                results.push({ systemId, systemName, success: res2.success, message: res2.message });
                continue;
            }
            ...
            fd.append('id',               activeSchedule.id);
            fd.append('system_id',        systemId);
            fd.append('title',            activeSchedule.title);
            fd.append('description',      activeSchedule.description || '');
            fd.append('start_datetime',   activeSchedule.start_datetime);
            fd.append('end_datetime',     activeSchedule.end_datetime);
            fd.append('status',           'Done');
            fd.append('change_to_online', 'yes');

            const r3   = await fetch('../backend/maintenance/save_maintenance.php', { method: 'POST', body: fd });
            const res3 = await r3.json();
            results.push({ systemId, systemName, success: res3.success, message: res3.message });
    ...
    // ── Update UI for each successful system ──
    let successCount = 0;
    results.forEach(r => {
        if (r.success) {
            successCount++;
            const card = document.querySelector(`.system-card[data-system-id="${r.systemId}"]`);
            if (card) {
                card.dataset.status = 'online';
                const badge = card.querySelector('.card-status-badge');
                if (badge) {
                    badge.outerHTML = '<div class="card-status-badge status-online">' +
                                      '<span class="status-indicator"></span>Online</div>';
                }
    ...
    // ── Reload side panel if open ──
    if (MaintenanceApp.sidePanelDate) {
        loadSidePanelSchedules(MaintenanceApp.sidePanelDate);
    }

    // ── Close modal + exit bulk mode ──
    closeBulkModal();
    if (BulkSchedule.active) toggleBulkMode();

    // ── Toast ──
    if (successCount === results.length) {
        showToast(`${successCount} system${successCount > 1 ? 's' : ''} marked as Online!`, 'success');
    } else {
        const failed = results.length - successCount;
        showToast(`${successCount} marked Online, ${failed} failed.`, successCount > 0 ? 'success' : 'error');
    }
}


/* ============================================================
   BULK MARK AS ONLINE — Confirmation Modal
   Opens when admin clicks "Mark X as Online" in floating bar.
   Shows each maintenance system's schedule details + checkboxes.
   ============================================================ */

async function openBulkMarkOnlineModal() {
    // Close the bulk scheduling modal if open
    const bulkModal = document.getElementById('bulkMaintenanceModal');
    if (bulkModal && bulkModal.classList.contains('show')) {
        bulkModal.classList.remove('show');
    }

    // Hide the floating bulk action bar while modal is open
    const bar = document.getElementById('bulkActionBar');
    if (bar) bar.classList.remove('visible');

    // Collect only maintenance systems from selection
    const maintenanceIds = [];
    BulkSchedule.selectedIds.forEach(id => {
        const card = document.querySelector(`.system-card[data-system-id="${id}"]`);
        if (card && card.dataset.status === 'maintenance') {
            maintenanceIds.push(id);
        }
    });

    if (maintenanceIds.length === 0) {
        showToast('No maintenance systems selected.', 'error');
        return;
    }

    // Show modal with loading state
    const modal = document.getElementById('bulkMarkOnlineModal');
    if (!modal) return;
    modal.classList.add('show');

    const listEl = document.getElementById('bulkMarkOnlineList');
    listEl.innerHTML = `
        <div class="side-panel-loading" style="padding:24px;">
            <div class="loading-spinner"></div>
            <span>Loading schedule details...</span>
        </div>`;

    // Fetch schedule details for each maintenance system
    const systemSchedules = [];
    for (const systemId of maintenanceIds) {
        const card       = document.querySelector(`.system-card[data-system-id="${systemId}"]`);
        const systemName = BulkSchedule.selectedNames[String(systemId)] || `System #${systemId}`;
        try {
            const res  = await fetch(`../backend/maintenance/get_maintenance.php?action=system&system_id=${systemId}`);
            const data = await res.json();
            const active = (data.data || []).find(s => s.status === 'In Progress' || s.status === 'Scheduled');
            systemSchedules.push({ systemId, systemName, schedule: active || null });
        } catch (e) {
            systemSchedules.push({ systemId, systemName, schedule: null });
        }
    }

    // Render system list with checkboxes + schedule details
    listEl.innerHTML = systemSchedules.map(item => `
        <div class="bmo-system-row" data-system-id="${item.systemId}">
            <label class="bmo-checkbox-wrap">
                <input type="checkbox"
                       class="bmo-system-check"
                       data-system-id="${item.systemId}"
                       checked>
                <span class="bmo-checkbox-indicator">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                        <polyline points="20 6 9 17 4 12"></polyline>
                    </svg>
                </span>
            </label>
            <div class="bmo-system-info">
                <div class="bmo-system-name">
                    ${escapeHtml(item.systemName)}
                    <span class="maint-status-badge maint-status-in-progress" style="font-size:10px;padding:2px 8px;flex-shrink:0;">
                        ● Maintenance
                    </span>
                </div>
                ${item.schedule ? `
                <div class="bmo-schedule-details">
                    <div class="bmo-schedule-title">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        ...
                        ${escapeHtml(item.schedule.title)}
                        <span class="maint-status-badge ${getStatusClass(item.schedule.status)}" style="font-size:10px;padding:2px 7px;margin-left:6px;">
                            ${escapeHtml(item.schedule.status)}
                        </span>
                    </div>
                    <div class="bmo-schedule-time">
                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10"></circle>
                            <polyline points="12 6 12 12 16 14"></polyline>
                        </svg>
                        ${formatDateDisplay(item.schedule.start_datetime)}
                        <span style="margin:0 4px;color:var(--gray-400)">→</span>
                        ${formatDateDisplay(item.schedule.end_datetime)}
                    </div>
                </div>` : `
                <div class="bmo-schedule-details bmo-no-schedule">
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <circle cx="12" cy="12" r="10"></circle>
                        <line x1="12" y1="8" x2="12" y2="12"></line>
                        <line x1="12" y1="16" x2="12.01" y2="16"></line>
                    </svg>
                    No active schedule found — will update system status directly.
                </div>`}
            </div>
        </div>
    `).join('');
    ...
    _updateBmoConfirmCount();

    // Listen for checkbox changes to update count
    listEl.querySelectorAll('.bmo-system-check').forEach(cb => {
        cb.addEventListener('change', _updateBmoConfirmCount);
    });

    // Store schedule data for confirm action
    modal._systemSchedules = systemSchedules;
}

function _updateBmoConfirmCount() {
    const checked = document.querySelectorAll('.bmo-system-check:checked').length;
    const countEl = document.getElementById('bmoConfirmCount');
    if (countEl) countEl.textContent = checked;
    const confirmBtn = document.getElementById('bmoConfirmBtn');
    if (confirmBtn) confirmBtn.disabled = checked === 0;
}

function closeBulkMarkOnlineModal() {
    const modal = document.getElementById('bulkMarkOnlineModal');
    if (modal) {
        modal.classList.remove('show');
        modal._systemSchedules = null;
    }

    // Restore floating bar if still in bulk mode
    if (BulkSchedule.active && BulkSchedule.selectedIds.size > 0) {
        const bar = document.getElementById('bulkActionBar');
        if (bar) bar.classList.add('visible');
    }
...
async function confirmBulkMarkOnline() {
    const modal = document.getElementById('bulkMarkOnlineModal');
    if (!modal || !modal._systemSchedules) return;

    // Get checked system IDs
    const checkedIds = Array.from(document.querySelectorAll('.bmo-system-check:checked'))
        .map(cb => parseInt(cb.dataset.systemId));
    ...
    for (const systemId of checkedIds) {
        const item = modal._systemSchedules.find(s => s.systemId === systemId);
        if (!item) continue;

        try {
            if (item.schedule) {
                // Mark schedule as Done + change_to_online=yes
                const fd = new FormData();
                fd.append('action',           'update');
                fd.append('id',               item.schedule.id);
                fd.append('system_id',        systemId);
                fd.append('title',            item.schedule.title);
                fd.append('description',      item.schedule.description || '');
                fd.append('start_datetime',   item.schedule.start_datetime);
                fd.append('end_datetime',     item.schedule.end_datetime);
                fd.append('status',           'Done');
                fd.append('change_to_online', 'yes');

                const r   = await fetch('../backend/maintenance/save_maintenance.php', { method: 'POST', body: fd });
                const res = await r.json();
                results.push({ systemId, systemName: item.systemName, success: res.success, message: res.message });
            } else {
                // No schedule — update system status directly
                const card   = document.querySelector(`.system-card[data-system-id="${systemId}"]`);
                const name   = card ? card.querySelector('.card-title')?.textContent.trim()  : item.systemName;
                const domain = card ? card.querySelector('.card-domain')?.textContent.trim() : '';
                ...
                const r   = await fetch('../backend/edit_system.php', { method: 'POST', body: fd });
                const res = await r.json();
                results.push({ systemId, systemName: item.systemName, success: res.success, message: res.message });
            }
        } catch (err) {
            results.push({ systemId, systemName: item.systemName, success: false, message: 'Network error' });
        }
    }
    ...
    // Close modals + exit bulk mode
    closeBulkMarkOnlineModal();
    if (BulkSchedule.active) toggleBulkMode();
