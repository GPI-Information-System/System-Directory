...
function closeAllDropdowns() {
  ...
  document
    .querySelectorAll(".filter-dropdown-menu")
    .forEach((menu) => menu.classList.remove("show"));
}
...
function openEditModal(
  ...
  document.getElementById("editExcludeHealthCheck").checked =
    excludeHealthCheck == 1;
  ...
  fetch("../backend/add_system.php", { method: "POST", body: formData })
    ...
    .then((text) => {
      if (!text || text.trim() === "") {
        showModalError(
          "addModal",
          "Logo file is too large. Maximum allowed size is 5 MB. Please resize or compress your image and try again.",
        );
        return;
      }
      let data;
...
function updateClearFiltersBtn() {
  ...
  const categoryActive =
    document.querySelector("[data-cat].active")?.dataset.cat !== "all";
...
function filterSystems(status) {
  const filterBtn = document.querySelector(
    '.btn-filter[onclick="toggleFilterDropdown(event)"]',
  );
  if (filterBtn) {
    const labelMap = {
      all: "Status Filter",
...
function loadDashboardChart() {
  fetch("../backend/get_analytics_data.php?action=systems_by_status")
    .then((response) => response.json())
    .then((data) => {
      if (data.success) renderDashboardChart(data.data);
    })
...
function markSystemOnline() {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML =
      '<span class="loading-spinner" style="width:13px;height:13px;border-width:2px;display:inline-block;margin-right:6px;vertical-align:middle;"></span> Setting Online...';
  }

  fetch(
    "../backend/maintenance/get_maintenance.php?action=system&system_id=" +
      systemId,
  )
    .then((r) => r.json())
    .then((data) => {
    ...
    .catch(() => {
      if (btn) {
        btn.disabled = false;
        btn.innerHTML =
          '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg> Mark as Online';
      }
      if (typeof showToast === "function")
        showToast("Network error. Please try again.", "error");
    });
...
function _onMarkOnlineSuccess(systemId) {
  ...
  if (card) {
    ...
    if (badge) {
      badge.outerHTML =
        '<div class="card-status-badge status-online"><span class="status-indicator"></span>Online</div>';
    }
